...
function unavailable() {
  ...
  function DisplayProduct(y){
    ...
    for(var i = 0; i < y.length; i++){
      document.getElementById("Display").innerHTML += "<div class=container d-flex justify-content-center mt-50 mb-50><div class= row><div class=col-md-10><div class=card card-body> <div class= media align-items-center align-items-lg-start text-center text-lg-left flex-column flex-lg-row><div class=mr-2 mb-3 mb-lg-0><img src="+y[i].Image +" width=150 height=150 alt=L></div><div class=media-body><h6 class=media-title font-weight-semibold> <a data-abc=true><b>"+y[i].Title+ "</b></a></h6><ul class=list-inline list-inline-dotted mb-3 mb-lg-2> <li class=list-inline-item><a  class=text-muted data-abc=true>"+y[i].Catagory+"</a></li> <li class=list-inline-item><a  class=text-muted data-abc=true>"+y[i].SubCat+"</a></li></ul><p class=mb-3>"+y[i].Descreption+"</p><ul class=list-inline list-inline-dotted mb-0> <li class=list-inline-item>"+y[i].State+"</li></ul></div><div class=mt-3 mt-lg-0 ml-lg-3 text-center><h3 class=mb-0 font-weight-semibold>"+y[i].Price+"</h3><div class=text-muted>"+y[i].Reviews+"</div><button type=button class=btn btn-warning mt-4 text-white style=background-color:gold;><i class=icon-cart-add mr-2></i>"+y[i].Cart+"</button></div></div></div></div></div></div>";
  }
}
